...
- [ ] 明确“结构化真相源”与“向量检索语料”的分层存储策略
- [ ] 为后续语音能力补充更完整的规范化语料留档，而不仅是当前 RAG 语料
- [ ] 引入真实语义 embedding，实现替换当前 `HashEmbeddingFunction` 的占位方案
- [ ] 增强 persona 构建逻辑，而不仅是固定模板文本
- [ ] 增强 few-shot 选择策略，提高风格复现质量
...
- 当前项目已完成“真实导出兼容 + 正式接入收口 + 基础验收文档 + 基础会话历史接入”的 P0/P1 主线
- 当前项目仍未完成“效果增强、真实 embedding、历史压缩与去重、语音扩展”等后续能力
- 后续开发优先顺序建议为：`效果增强` → `历史增强` → `语音扩展`
